...
ScheduleMeJob.prototype.start = function() {
  ...
  if(this.when.getMonth) {
    ...
    // Execute on date
    this.dateTimeout = setTimeout(_.bind(this.jobStart, this), msUntilDate);
  }
};
 ...
 * Wrap for job start
 */
ScheduleMeJob.prototype.jobStart = function() {
  var uniqueId = uuid.v4();

  if(!this.options.no_emit) {
    this.parent.emit('jobStart', {name: this.name, job_id:uniqueId, time:new Date()});
  }

  this.job(_.bind(_.partial(this.jobEnded, uniqueId), this));
};

/**
 * Callback when job ends
 */
ScheduleMeJob.prototype.jobEnded = function(id, err, data) {
